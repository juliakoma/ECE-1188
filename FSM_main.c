...
void FSM_Update(void) {
    ...
    switch (currentState) {
        ...
        case MILD_LEFT:
            ...
            Motor_Left(1225, 1200);
            //Motor_Left(2500, 2400);
            //Motor_Left(1025, 1000);
        ...
        case MILD_RIGHT:
            sensorData = Read_Line_Position();
            Motor_Right(1200, 1225);
            //Motor_Right(1025, 1000);
            if(sensorData >= -800) {
                currentState = FORWARD;
            }
            break;
...
int main(void) {
    ...
    while (1) {
        FSM_Update();
        Clock_Delay1ms(20);  // Update FSM every 20ms
    }
}
